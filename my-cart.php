...
if(isset($_POST['submit'])){
		...
		foreach($_POST['quantity'] as $key => $val){
			...
			echo "<script>alert('Ваша корзина обновлена');</script>";
		}
	}
...
// code for billing address updation
	if(isset($_POST['update']))
	{
		...
		if($query)
		{
echo "<script>alert('Адрес Магазина был обновлен');</script>";
		}
	}
...
// code for Shipping address updation
	if(isset($_POST['shipupdate']))
	{
		...
		if($query)
		{
echo "<script>alert('Адрес был обновлен');</script>";
		}
	}
		...
		<table class="table table-bordered">
			<thead>
				<tr>
					...
					<th class="cart-description item">карта</th>
					<th class="cart-product-name item">Имя</th>
			
					<th class="cart-qty item">Качество</th>
					<th class="cart-sub-total item">Цена за единицу</th>
					<th class="cart-sub-total item">Плата за доставку</th>
...
?>
								<div class="reviews">
									( <?php echo htmlentities($num);?> Обзоры )
								</div>
								<?php } ?>
							</div>
						</div><!-- /.row -->
					...
					<td class="cart-product-sub-total"><span class="cart-sub-total-price"><?php echo "Руб"." ".$row['productPrice']; ?>.00</span></td>
<td class="cart-product-sub-total"><span class="cart-sub-total-price"><?php echo "Руб"." ".$row['shippingCharge']; ?>.00</span></td>
...
$_SESSION['pid']=$pdtid;
	...
	<table class="table table-bordered">
		<thead>
			<tr>
				<th>
					<span class="estimate-title">Ваш адрес</span>
				</th>
			</tr>
		</thead>
		<tbody>
...
<div class="form-group">
					    <label class="info-title" for="Billing Address">Магазин<span>*</span></label>
						...
						<div class="form-group">
					    <label class="info-title" for="Billing State ">Область  <span>*</span></label>
			 <input type="text" class="form-control unicase-form-control text-input" id="bilingstate" name="bilingstate" value="<?php echo $row['billingState'];?>" required>
					  ...
					  <div class="form-group">
					    <label class="info-title" for="Billing City">Город <span>*</span></label>
...
<div class="col-md-4 col-sm-12 estimate-ship-tax">
	<table class="table table-bordered">
		<thead>
			<tr>
				<th>
					<span class="estimate-title">Адрес магазина</span>
				</th>
			</tr>
		</thead>
		<tbody>
...
<div class="form-group">
					    <label class="info-title" for="Shipping Address">Ваш адрес<span>*</span></label>
			...
			<tr>
				<th>
					...
					<div class="cart-grand-total">
						Итого<span class="inner-left-md"><?php echo $_SESSION['tp']="$totalprice". ".00"; ?></span>
					</div>
				</th>
			</tr>
		</thead><!-- /thead -->
				...
				<tr>
					<td>
						<div class="cart-checkout-btn pull-right">
							<button type="submit" name="ordersubmit" class="btn btn-primary">Перейти к покупке</button>
